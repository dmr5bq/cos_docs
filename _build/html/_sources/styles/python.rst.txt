...
Use single quotes for strings, e.g., ``str_literal = 'This is a string.'``, unless doing so requires lots of escaping.

Avoid one-letter variables (esp. ``l``, ``O``, ``I``).

*Exception*: In very short blocks, when the meaning is clearly visible from the immediate context, e.g., indices like ``i``, ``j``, or ``k`` and simple integers with clear meaning like ``h``, ``k``, ``m``, ``n`` or coordinate integers like ``x``, ``y``, ``z``.

.. code-block:: python

	# Another clear acceptable use:
	# using a for-loop to iterate over some collection
    for e in elements:
        e.mutate()

Avoid redundant labeling when possible.
...
.. code-block:: python
    ...
    # No, note the slight redundancy in naming
    import audio

    core = audio.AudioCore() 
    controller = audio.AudioController()

Prefer "reverse notation" for clarity in identifier naming.
...
Avoid getter and setter methods; access fields directly instead.
...
Use 4 spaces instead of tabs. You may need to change the settings in your
...
.. code-block:: python
    ...
    # Yes, importing entire module or submodule
    import canteen
    import canteen.sessions
    from canteen import sessions

    # No, importing one specific symbol or method
...
.. code-block:: python
    ...
    from my.very.distant.module import foo

    foo = foo # shows explicit reasoning for foo import
...
80-100 characters is a good range for maximum line lengths.

Use parentheses for line continuations when necessary.
